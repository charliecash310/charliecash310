# About Me

Security practitioner focused on defensive security, threat hunting, and vulnerability management.  
I build labs, break systems, analyze telemetry, and document failures until they stop failing.

This repository is a living record of hands-on work across blue team, red team, and infrastructure security.

---

## Technical Skills

- **Scripting & Automation:** Bash, Python  
- **Web Technologies:** HTML, CSS, JavaScript (security testing & lab environments)  
- **CLI & OS Proficiency:** Linux-first workflows, Windows administration

---

## Environments Used

- **Linux:** Kali, Parrot, Ubuntu, Debian  
- **Windows:** XP, Vista, 7, 10, 11  

---

## Training & Coursework

### Google Cybersecurity
- [Activities](https://github.com/charliecash310/Google-Cybsersecurity)

### Cisco Networking Academy
- [Ethical Hacker Course](https://github.com/charliecash310/Cisco-Networking-Academy)
- [Network Defense](https://github.com/charliecash310/Cisco-Network-Academy---Network-Defense)
- Endpoint Detection
- IT Support
- Kali Linux & Offensive Security

---

## Applied Projects

### Azure: Virtual Machine & Web Server Deployment
- [View Project](https://github.com/charliecash310/Azure---VM---Webserver/blob/main/README.md)
- Deployed and secured a cloud-based VM hosting a web server
- Documented configuration, exposure, and security considerations

### SOC Analyst Lab // Blue Team & Red Team Exercise
- [View Project](https://github.com/charliecash310/SOC-LAB-Blue-Team-Red-Team)
- Built a multi-VM lab simulating attacker and defender activity
- Generated malicious telemetry and validated detections
- Analyzed alerts, logs, and persistence mechanisms

### Vulnsite
- [View Project](https://github.com/charliecash310/vulnsite-demo)
- Intentionally vulnerable application for web security testing and exploitation

---

## Vulnerability Management

- [Vulnerability Management Program](https://github.com/charliecash310/Vulnerability-Management-Program-)
- [Applying DISA STIGs with PowerShell (Windows 11)](https://github.com/charliecash310/DISA-STIG-Applying-STIGs-with-PowerShell-win11-stig-)

---

## Threat Hunting

Focused on persistence mechanisms, PowerShell abuse, and behavioral detection.

- [Silent PowerShell Dropper with Persistence](https://github.com/charliecash310/silent-dropper-lab)
- [Threat Hunt Community Events (KC7)](https://github.com/charliecash310/Threat-Hunt-Reports)

---

## Applied AI in Cybersecurity

- [ChatGPT for Cybersecurity](https://github.com/charliecash310/OpenAI-Cookbook)
- Using AI to assist with threat analysis, documentation, and automation workflows

---
